...
void VarDecl::Check() {
    ...
    if (assignTo != NULL) {
        ...
        if ( !(assignTo->GetType()->IsConvertibleTo(type)) )
            ReportError::InvalidInitialization(GetIdentifier(),type,assignTo->GetType());
    }


    // Need to handle Cascading Error here ??? 
}
...
void FnDecl::Check() {
    ...
    StmtBlock* stmtblk = dynamic_cast<StmtBlock*> (body);
    stmtblk -> Check(formals);
    
    // Check for return
    if(hasReturned == false && returnType != Type::voidType)
